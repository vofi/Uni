...
public class Game {
	...
	//NEW:
	public void setSquareToLargeSquare(int position,Game game)
	{
		LargeSquare tmpLargeSquare=new LargeSquare(game,position);
		this.setSquare(position, tmpLargeSquare);
		
		this.initSquare(position,tmpLargeSquare);
		
	}
	
	public void setSquareToTrapDoor(int transport,Game game, int position, int maxPlayerOnTrapDoor)
	{
		TrapDoor tmpTrapDoor=new TrapDoor(transport,game,position,maxPlayerOnTrapDoor);
		this.setSquare(position,tmpTrapDoor);
		this.initSquare(position, tmpTrapDoor);
	}
	...
	//Optional
	public ISquare findBeforeLargeSquare(int position)
	{
		for(int i=squares.size()-1;i>=0;i--)
		{
			if (squares.get(i).isLargeSquare()==true)
