/* TODO: DONE Task (b) Please fill in the following lines, then remove this line.
 *
 * author(s):   Jonas von Felten (10-105-427)              
...
/* TODO Task (h) complete printInstruction here */  
	...
	switch (o.type){
	case iType: 
		printf("%-5s, %02d, %02d, 0x%04x", o.name, i->i.rt, i->i.rs, i->i.immediate); 
		printf("\n");
		break;
	case jType: 
		printf("%-5s, 0x%08x" , o.name,i->j.address); 
		printf("\n");
		break;
	case rType: 
		f = functions[i->r.funct];		
		printf("%-5s, %02i, %02i , %02i, 0x%04x", f.name, i->r.rt, i->r.rd, i->r.rs, i->r.shamt); 
		printf("\n");
		break;
	case specialType: 
		printf("%-5s", o.name);
